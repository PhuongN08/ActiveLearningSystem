    ...
    public class CourseListService : ICourseListService
    {
     ...
     int pageSize = 5)
        ...
        public async Task<bool> CreateCourseAsync(CourseCreateVM courseVM, IFormFile image, int accountId)
        {
            ...
            // Kiểm tra ký tự hợp lệ trong tên khóa học
            ...
            // Chuẩn hóa tên khóa học để kiểm tra trùng lặp (không phân biệt hoa thường)
            var normalizedName = courseVM.CourseName.Trim().ToLower();

            var existingName = await _context.Courses
                .AnyAsync(c => c.CourseName.ToLower() == normalizedName);
