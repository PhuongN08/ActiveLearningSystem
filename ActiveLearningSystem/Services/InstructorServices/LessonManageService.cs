...
namespace ActiveLearningSystem.Services.InstructorServices
{
    public class LessonManageService : ILessonManageService
    {
        ...
        public async Task<bool> UpdateLessonAsync(int lessonId, LessonUpdateVM lessonUpdateVM)
        {
            ...
            // Kiểm tra VideoNum trùng
            bool isVideoNumDuplicate = await _context.Lessons
                .AnyAsync(l => l.Id != lessonId
                               && l.ModuleId == lesson.ModuleId
                               && l.VideoNum == lessonUpdateVM.VideoNum);
            if (isVideoNumDuplicate)
                throw new Exception("VideoNum không được trùng với các lesson khác trong cùng module.");
