...
namespace ActiveLearningSystem.Controllers.InstructorController
{
    ...
    public class ManageModuleController : ControllerBase
    {
        ...
        [Authorize(Roles = "Instructor")]
        [HttpPut("update-status/{moduleId}")]
        public IActionResult UpdateModuleStatus(int moduleId, [FromQuery] bool status)
        {
            try
            {
                _moduleListService.UpdateModuleStatus(moduleId, status);

                var action = status ? "đã được hiển thị" : "đã bị ẩn";
                return Content($"Mô-đun {action} thành công.");
            }
            catch (Exception ex)
            {
