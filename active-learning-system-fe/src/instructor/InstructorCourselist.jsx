...
const InstructorCourselist = () => {
    ...
    const [stats, setStats] = useState({ total: 0, students: 0, hours: 0, rating: 0 });
    ...
    useEffect(() => {
        if (showCreateModal || showEditModal) {
            ...
            Promise.all([
                ...
                setDropdownError("Không thể tải danh mục hoặc lớp học.");
                setCategories([]);
                setClasses([]);
            }).finally(() => {
    ...
    useEffect(() => {
        const fetchCourses = async () => {
            ...
            try {
                ...
                setStats({
                    total: data.length,
                    students: data.reduce((sum, c) => sum + (c.students || 0), 0),
                    hours: data.reduce((sum, c) => sum + (c.hours || 0), 0),
                    rating: data.length ? (data.reduce((sum, c) => sum + (c.averageRating || 0), 0) / data.length).toFixed(1) : 0
                });
            } catch (err) {
    ...
    function canContinue() {
        // Validate: name, description, category, class
